...
class PlanOption(BaseModel):
    option_id: int
    # 🌟 ตัวแปรใหม่ให้ AI เลือกโหมดสับราง
    route_mode: str = Field(description="Must be exactly one of: 'parallel', 'sequential_weather_first', or 'sequential_stocks_first'")
    tasks: Dict[str, str] = Field(description="Use the agent's name as the key (weather_agent, stocks_agent) and the prompt/command as the value (Strict rule: If a specific agent is not needed, do not include its key).")
